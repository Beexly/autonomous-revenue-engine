...
8. **If node** - continue to referral only when the review row has `status` equal to `submitted` or `published`, and `rating >= 4`; otherwise stop after the reminder path and create a CRM follow-up or service-recovery task if needed.
